...
INSERT INTO seminar_4.cast VALUES ('Милли', 'Олкок', '2000-04-11'),
                                  ('Мэтт', 'Смит', '1982-10-28'),
                                  ('Эмма', 'Д''Арси', '1992-06-27'),
                                  ('Оливия', 'Кук', '1993-12-27'),
                                  ('Эмили', 'Кэри', '2003-04-30');

INSERT INTO seminar_4.cast VALUES ('Юэн', 'Митчелл'),
                                  ('Пэдди', 'Консидайн'),
                                  ('Ив', 'Бест');
...
ALTER TABLE seminar_4.cast ADD COLUMN id SERIAL PRIMARY KEY;
...
ALTER TABLE seminar_4.cast ALTER COLUMN birth_date SET DEFAULT DATE '1900-01-01';
...
-- 7. Создайте в схеме seminar_4 таблицу characters с ненулевым полем name, внешним ключом actor_id,
-- ссылающимся на поле id из таблицы seminar_4.cast, текстовым полем comment, полем sex, которое
-- может принимать значения 'male' или 'female'.

CREATE TABLE seminar_4.characters (
    name TEXT NOT NULL,
    --actor_id INT REFERENCES seminar_4.cast(id)
    actor_id INT,
    comment TEXT,
    sex TEXT CHECK (sex IN ('male', 'female')),
    CONSTRAINT FK_cast FOREIGN KEY (actor_id) REFERENCES seminar_4.cast(id)
);


-- 8. Запустите следующие операции вставки в таблицу:

INSERT INTO seminar_4.characters (name, actor_id, comment, sex) VALUES ('Рейнира Таргариен', 1, 'Юная принцесса', 'female'),
...
SELECT *
FROM seminar_4.characters;

-- 9. Попробуйте добавить в таблицу:
SELECT *
FROM seminar_4.cast;
    -- строку с actor_id, который не встречается в таблице seminar_4.cast;
    INSERT INTO seminar_4.characters (name, actor_id, comment, sex) VALUES ('Татьяна Михеева', 8, '', 'female');
    -- не добавится

    -- строку с пустым полем sex: ('Эймонд Таргариен', 9);
    INSERT INTO seminar_4.characters (name, actor_id, comment) VALUES ('Эймонд Таргариен', 9, '');


    -- строку с пустым полем actor_id: ('Эймонд Таргариен');
    INSERT INTO seminar_4.characters (name) VALUES ('Эймонд Таргариен');
...
DELETE FROM seminar_4.characters
WHERE sex IS NULL;

ALTER TABLE seminar_4.characters ADD PRIMARY KEY (name, actor_id);

SELECT *
FROM seminar_4.characters;

-- 11. Удалите из таблицы seminar_4.cast актеров, игравших юных героев (содержащих в комментарии слово
-- ‘юный’ или ‘юная’) так, чтобы все их герои также исчезли из таблицы seminar_4.characters.
-- Возможно придется удалить созданное ограничение на внешний ключ и задать новое.

ALTER TABLE seminar_4.characters DROP CONSTRAINT FK_cast;

ALTER TABLE seminar_4.characters ADD CONSTRAINT FK_cast FOREIGN KEY (actor_id) REFERENCES seminar_4.cast(id)
ON DELETE CASCADE;
